...
When adding an internal page (Flight 5+): add a `host → file` entry to `INTERNAL_PAGES`, extend `isInternalPageUrl`'s allowlist, and open it via the trusted `createTab` path — never by relaxing the web gates.
