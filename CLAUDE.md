...
- **`webContents.debugger` lives ONLY in `observe.js`'s `readAxTree`.** There is no pure-JS path to the platform accessibility tree, so the a11y read is the engine's sole debugger user: it attaches the in-process CDP debugger on demand, holds a synchronous single-client lock keyed on `wcId`, runs the `Accessibility.*` commands, and detaches in a `finally` (never left attached). A busy debugger (DevTools open, or a second client) returns a discriminated `{ automation: 'debugger-unavailable', … }` refusal rather than throwing. Every other op and module stays debugger-free; keep it that way — do not add `webContents.debugger` use elsewhere.
...
- **Nothing ships until Flight 4.** The automation surface is **ungated** until Flight 4 — it must not be exposed in any released build before then. No release cuts this surface live.
