...
// old hybrid require-or-global resolution (RESOLVED_BURNER, M06 Flight 2 DD8) is
// retired. Historical note (Leg 3 D1): as a classic script this file had to avoid
// re-declaring `const BURNER` (classic <script> tags share ONE top-level lexical
// scope with burner.js). Module scope now isolates top-level declarations, so the
// import binding below is structurally collision-free.
 ...
 */
export function buildContainerModel(containers, defaultId) {
  ...
  /** @type {{ id: string, label: string, color?: string, isDefault?: boolean }} */
  const burnerItem = { id: 'action:burner', label: `${BURNER.name} tab (evaporates)`, color: BURNER.color };
