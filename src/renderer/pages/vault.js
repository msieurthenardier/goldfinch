...
function init() {
  ...
  // The restore/export modal cluster (Flight 2 Leg 1 / DD4 — the M18 F3 debt, paid here):
  // extracted out of this file into a sibling injected-deps controller, the createVaultBrowserImport
  // precedent above. jarRows/jarVaultPresence are threaded as GETTERS (never a snapshot) — both
  // mutate in refresh(), and the mapping modal must read them live at open time. `setNotice`
  // carries the SECOND state coupling (pendingNotice, below) out of the controller — it is
  // written inside the moved openExportModal but read/cleared only here, in render().
  const restore = createVaultRestoreController({
    bridge,
    dom: { el, button, iconButton, openModal, appendOption },
    getJarRows: () => jarRows,
    getJarVaultPresence: () => jarVaultPresence,
    refresh,
    setNotice: (text) => {
      pendingNotice = text;
    }
  ...
  function buildNotSetUp() {
    ...
    actions.appendChild(
      button('Import a vault bundle', 'vault-btn', () => restore.openImportPickModal({ fresh: true }))
    );
    ...
    // Settings section uses, gated on the controller-owned held record.
    if (restore.heldRecord()) {
      section.appendChild(button('Resume restore…', 'vault-btn', () => restore.openMapping(restore.heldRecord(), [])));
    }
  ...
  function buildImportExportSection(vaults) {
    ...
    if (restore.heldRecord()) {
      ...
      resumeRow.appendChild(
        button('Resume restore…', 'vault-btn', () => restore.openMapping(restore.heldRecord(), vaults))
      );
      section.appendChild(resumeRow);
    }
    const row = el('div', 'vault-settings-row');
    row.appendChild(button('Import…', 'vault-btn', () => restore.openImportPickModal()));
    ...
    row.appendChild(button('Export…', 'vault-btn', () => restore.openExportModal(vaults)));
    section.appendChild(row);
    return section;
  }
  ...
  const importLabelsHandle = bridge.onVaultImportLabelsReady(() => {
    restore.handleLabelsReady(lastViewVaults);
  });
  ...
  window.addEventListener('pagehide', () => restore.dropHeldOnPagehide(), { once: true });
