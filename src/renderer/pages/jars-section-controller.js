export function createJarsSections(deps) {
  ...
  function buildJarSection(row) {
    ...
    function buildPanelContent(panelId, panelEl) {
      ...
      if (panelId === 'history') {
        ...
        panelRefreshButtons.set(
          'cookies',
          buildPanelRefreshButton(() => cookiesPanel?.refresh())
        );
        ...
        panelRefreshButtons.set(
          'site-data',
          buildPanelRefreshButton(() => siteDataPanel?.refresh())
        );
      }
    }
  ...
  const DELETE_COPY =
    'Deletes this jar and wipes its cookies, site storage, and cache. Open tabs in this jar will close.';
  ...
  function trySelectHash(hash, exactHashTarget) {
    const targetId = exactHashTarget(
      hash,
      new Set(
        Array.from(sectionMap.values()).flatMap((refs) =>
