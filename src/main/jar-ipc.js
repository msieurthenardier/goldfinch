...
//
// Trust domain (DD7): the mutating channels are deliberately CHROME-TRUSTED —
// bare `ipcMain.handle`, the same domain as the picker's `new-container-create`
// (web webviews have no ipcRenderer.invoke, so only the chrome + internal guest
// can reach IPC at all). Flight 3 adds internal-origin-gated variants
// (registerInternalHandler) for the goldfinch:// management page; do NOT wrap
// these with it — that would break the file:// chrome consumer.
//
...
// precedent — and it keeps the six handlers out of the main.js god file).
...
function registerJarIpc({ ipcMain, jars, session, rerollSeed, revokeJarKey, settings, broadcast }) {
  ...
  ipcMain.handle('jars-rename', (_e, p) => {
    if (p === null || typeof p !== 'object') return null;
  ...
  ipcMain.handle('jars-set-default', (_e, p) => {
  ...
  ipcMain.handle('jars-get-default', () => jars.getDefault());
  ...
  ipcMain.handle('jars-remove', async (_e, p) => {
