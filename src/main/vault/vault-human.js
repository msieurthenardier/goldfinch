...
/**
 * AC15 (Mission 21, Flight 4, Leg 2 — password-field-roles): zeroize+delete a
 * held record's `currentPassword` buffer once the disposition (update-by-
 * password-match, or not) has been computed — it is never needed for save
 * (`captureSave` persists `rec.password`, the NEW value, never
 * `rec.currentPassword`), so this shortens the secret's held lifetime rather
 * than waiting for `dropCapture`'s end-of-life sweep. Called from exactly two
 * sites, both immediately after `applyUsernameDowngrade(rec, disposeCapture(rec))`
 * returns: `capture()`'s unlocked branch and `captureFinalize`'s login
 * dispatch. A missing/falsy `currentPassword` is a no-op.
 * @param {CaptureRecord} rec
 */
function zeroizeCurrentPassword(rec) {
  if (rec.currentPassword) {
    rec.currentPassword.fill(0);
    delete rec.currentPassword;
  }
}
 ...
 * @property {(arg: { wcId: number, candidates: string[] }) => void} [fillGeneratedDelegate]
 *   the GENERATE-IN-PICKER main→preload fill effect (Mission 21, Flight 4, Leg 3 —
 *   generate-in-picker, AC13 — `send('vault-fill-generated', { candidates })`). Same
 *   optional-refuses-rather-than-drops shape as `fillCardDelegate`.
 ...
 * @property {Buffer} [currentPassword]  login only (Mission 21, Flight 4, Leg 2 —
 *   password-field-roles, DD3a/DD4): the CURRENT password, present only when the capturing
 *   gesture's login scope classified with a provenanced `current` field (a rotation form).
 *   Read once by `disposeCapture`'s password-match rule, then zeroized+deleted right after
 *   disposition is computed (AC15) — it is never needed for save (`captureSave` persists
 *   `rec.password`, the NEW value). Covered by `dropCapture`'s zeroize-every-Buffer sweep
 *   (LD7) with no edit needed there.
 * @property {boolean} [matchedByPassword]  login only (DD4): stamped `true` by `disposeCapture`
 *   when the update disposition was decided by the current-password match rather than the
 *   origin+username rule — `applyUsernameDowngrade` returns such a record's model unchanged
 *   (DD3c's downgrade protects the USERNAME-keyed match only; a password-keyed match never
 *   reads the username at all).
...
function createVaultHuman(deps) {
   ...
   * DD5/AC13). Gates, in order: (1) the vault is set up; (2) `tabJarFor(wcId)`
   * resolves a PERSISTENT jar; (3) `tabOriginFor(wcId)` is a non-null web
   * origin. Deliberately does NOT require unlocked — DD5's whole point is that
   * Generate is reachable from a locked picker.
   *
   * `candidates` are already generated by the CALLER (main.js's IPC handler
   * re-runs `sanitizeGenerateConstraints` + `generateCandidates` itself, never
   * trusting the chrome's earlier `canGenerate`); an empty/non-array
   * `candidates` refuses `'unsatisfiable'`. On success, hands
   * `{ wcId, candidates }` to `deps.fillGeneratedDelegate` (the main->preload
   * effect — `webContents.fromId(wcId)?.send('vault-fill-generated',
   * { candidates })`) and returns `{ filled: true }`. The return NEVER carries
   * a candidate — the chrome never receives or holds the generated password.
   * @param {{ wcId: number, candidates: string[] }} arg
   * @returns {{ filled: boolean, reason?: string }}
   */
  function fillGenerated({ wcId, candidates }) {
    const store = deps.getVaultStore();
    const jar = tabJarFor(wcId);
    const origin = tabOriginFor(wcId);
    if (!store.isSetUp() || !jar || !origin) return { filled: false, reason: 'ineligible' };
    if (!Array.isArray(candidates) || candidates.length === 0) return { filled: false, reason: 'unsatisfiable' };
    if (!deps.fillGeneratedDelegate) return { filled: false, reason: 'ineligible' };
   ...
   *
   * DD4 (Mission 21, Flight 4, Leg 2 — password-field-roles): tried FIRST, ahead of the
   * origin+username rule below. When `rec.currentPassword` is non-empty (a rotation scope
   * carried a provenanced `current` field), read each DISTINCT vault's full items ONCE
   * among the reachable rows (never once per row) and find every reachable row whose
   * STORED password equals the current value byte-for-byte, AND — when the capture itself
   * carried a provenanced username — whose normalized username agrees. Exactly ONE
   * candidate → `update` to it, `rec.matchedByPassword = true`, and `rec.username` is
   * REASSIGNED to that row's own normalized username (not only the returned model's
   * `username`) — `captureSave`'s login-update branch writes `username: rec.username` over
   * the existing item, so leaving it null would silently blank a stored username on every
   * no-username-field rotation. Zero or several candidates fall through to today's
   * origin+username rule (never guess among several).
   * @param {CaptureRecord} rec
  ...
  function disposeCapture(rec) {
    ...
    // DD4: never match an empty current value (a login-scope with a `current`
    // role but nothing provenanced there never carries `rec.currentPassword`
    // at all — see planLogin's own gate — but guard on length too, defensively).
    if (rec.currentPassword && rec.currentPassword.length > 0) {
      const currentValue = rec.currentPassword.toString('utf8');
      // Batch: read each distinct vault's full items ONCE (never once per row).
      /** @type {Map<string, any[]>} */
      const itemsByVaultId = new Map();
      for (const row of reachable) {
        if (itemsByVaultId.has(row.vaultId)) continue;
        let items;
        try {
          items = store.listItems(row.vaultId);
        } catch {
          items = [];
        }
        itemsByVaultId.set(row.vaultId, items);
      }
      const candidates = [];
      for (const row of reachable) {
        const items = itemsByVaultId.get(row.vaultId) || [];
        const full = items.find((/** @type {any} */ i) => i.id === row.id);
        if (!full || full.password !== currentValue) continue;
        if (rec.username != null && normUsername(full.username) !== rec.username) continue;
        candidates.push({ row, full });
      }
      if (candidates.length === 1) {
        const { row, full } = candidates[0];
        // The existing unchanged guard: the row is a no-op update iff its
        // stored password already equals the NEW value being saved.
        if (full.password === rec.password.toString('utf8')) {
          return null;
        }
        rec.mode = 'update';
        rec.vaultId = row.vaultId;
        rec.itemId = row.id;
        rec.choices = [];
        rec.matchedByPassword = true;
        rec.username = normUsername(full.username);
        return {
          origin: rec.origin,
          username: rec.username,
          mode: 'update',
          defaultVaultId: row.vaultId,
          choices: []
        };
      }
      // zero or several candidates → fall through to today's origin+username rule.
    }
   ...
   *
   * DD4 EXEMPTION (Mission 21, Flight 4, Leg 2 — password-field-roles): a record whose
   * update was decided by `disposeCapture`'s current-password match
   * (`rec.matchedByPassword === true`) is returned UNCHANGED — checked FIRST, before the
   * username checks below. DD3c protects the USERNAME-keyed match from an unprovenanced
   * username; a password-keyed match never reads the username at all, so applying the
   * downgrade here would re-file a correct rotation as a duplicate `save` whenever the form
   * shows a read-only prefilled username (the common change-password shape).
   * @param {CaptureRecord} rec
  ...
  function applyUsernameDowngrade(rec, model) {
    if (rec.matchedByPassword === true) return model;
   ...
   * `currentPasswordBytes` (Mission 21, Flight 4, Leg 2 — password-field-roles, DD3a/DD4) is
   * OPTIONAL — see `holdGestureLogin`'s own doc for the acceptance/zeroize rules, mirrored here.
   * @param {{ wcId: number, username: any, passwordBytes: any, usernameDetected?: boolean, origin?: string|null, jar?: {id: string}|null, currentPasswordBytes?: any }} arg
  ...
  function capture({
    ...
    // AC15b (Mission 21, Flight 4, Leg 2 — password-field-roles): the OPTIONAL
    // current-password bytes are zeroized on EVERY path, symmetric with `bytes`.
    const currentBytes = currentPasswordBytes instanceof Uint8Array ? currentPasswordBytes : null;
    ...
    const currentPassword = currentBytes ? Buffer.from(currentBytes) : undefined;
    if (currentBytes) currentBytes.fill(0);
    ...
    // (called by the chrome after a successful unlock) — `rec.currentPassword` (when present)
    // stays held for that deferred pass; it is not zeroized here.
    if (!store.isUnlocked()) {
      rec.mode = 'locked';
    ...
    // make no offer. AC15: the current-password buffer is no longer needed once disposition
    // is computed — zeroize+delete it here, whichever way disposition came out.
    const model = applyUsernameDowngrade(rec, disposeCapture(rec));
    zeroizeCurrentPassword(rec);
    if (!model) {
   ...
   *
   * `currentPasswordBytes` (Mission 21, Flight 4, Leg 2 — password-field-roles, DD3a/DD4)
   * is OPTIONAL: only a `Uint8Array` is accepted (anything else is ignored, matching
   * `passwordBytes`'s own coercion); when present it is copied into `rec.currentPassword`
   * (a Buffer) for `disposeCapture`'s password-match rule, and the incoming array is
   * zeroized on EVERY path, including the gate-refusal early return.
   * @param {{ wcId: number, username: any, usernameDetected: boolean, passwordBytes: any, currentPasswordBytes?: any }} arg
   * @returns {{ captureId: string } | null}
   */
  function holdGestureLogin({ wcId, username, usernameDetected, passwordBytes, currentPasswordBytes }) {
    const store = deps.getVaultStore();
    const bytes = passwordBytes instanceof Uint8Array ? passwordBytes : null;
    const currentBytes = currentPasswordBytes instanceof Uint8Array ? currentPasswordBytes : null;
    const origin = tabOriginFor(wcId);
    ...
    for (const [pendingId, rec] of pending) {
      const outcome = dispatchByFamily(rec, {
        login: () => {
          ...
          // Mission 21, Flight 4, Leg 2 (password-field-roles): a pending
          // record's OPTIONAL `currentPassword` is copied the same way,
          // before the pending record is dropped (the same aliasing-hazard
          // discipline `rec.password` already follows above).
          const currentPasswordCopy = rec.currentPassword ? Buffer.from(rec.currentPassword) : undefined;
          ...
          return capture({
            ...
            origin,
            jar: { id: jarId },
            currentPasswordBytes: currentPasswordCopy
          });
        },
        card: () => {
  ...
  function captureFinalize(captureId) {
    ...
    const model = dispatchByFamily(rec, {
      login: () => {
        // AC15: the second of the two zeroize sites — immediately after
        // disposition is computed, whichever way it came out.
        const m = applyUsernameDowngrade(rec, disposeCapture(rec));
        zeroizeCurrentPassword(rec);
        return m;
      },
