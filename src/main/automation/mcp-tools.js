...
const DRIVE_TOOLS = [
  {
    name: 'enumerateTabs',
    description:
      "List all drivable (dom-ready) tabs across ALL windows as an array of { wcId, url, title, jarId, active, windowId, loadState, loadError, security }. windowId is stamped from the window registry, which is authoritative for ownership. A window whose chrome has not finished booting contributes ZERO rows — call enumerateWindows and poll until every `booted` is true if a total census is required. Admin listings include the internal goldfinch:// tabs; jar-key listings never do (session filter) — a jar key sees all windows' tabs for its own jar, never the window topology. Script-opened POPUP windows appear as extra rows (after the tab rows) marked `popup: true`, with `active: false` and windowId = the OWNER window's; a jar key sees only popups whose session is its own jar (a burner-opened popup is admin-visible only). Popup wcIds are drivable like tab wcIds. `loadState` is 'ok', 'failed', or 'cert-blocked' (Mission 20; the enum grows in later flights); `loadError` is `{ code, name }` when failed/cert-blocked, else null. `security` (Mission 20 Flight 2) is one of 'secure', 'insecure', 'overridden', 'internal', 'none' — the committed top-frame origin's TLS trust state; 'none' for about:blank, an unparseable URL, or a failed/cert-blocked load.",
