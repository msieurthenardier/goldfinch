...
clears everything AND auto-reloads the jar's open tabs (the `jar-wiped` broadcast
sweep — observed via an in-memory window marker that survives if and only if no
reload happened), and the Burner identity plus unknown ids are rejected (pinning the
partition-less Burner identity shape — the `jar-delete-closes-tabs` run-1 Validator
carry-forward). Also pins burner STORAGE isolation directly (cookie written in one
burner tab not readable in another) — closing the `popup-jar-inheritance` Validator
carry-forward, which noted that only partition NAMING distinctness was asserted.
Real-environment behavior spanning the twin-registered IPC handlers, Electron
session storage, the broadcast path, and the chrome renderer's reload sweep — no
unit seam covers the cross-process composition.
...
| 5 | Via chrome-target `evaluate`: `window.goldfinch.jarsWipe({ id: 'personal' })`. Settle (~1-2s), then via `evaluate` on the personal tab (same recorded `wcId`): read `window.__bt_alive` and `document.cookie`. Enumerate tabs. | The call resolves `{ ok: true }`. `window.__bt_alive` is `undefined` — the tab RELOADED (in-memory expando gone), driven by the `jar-wiped` broadcast sweep. `document.cookie` contains no `bt_probe`. The tab's `wcId` is unchanged in the enumeration (reloaded in place, not closed/reopened) and its `jarId` is still `personal`. |
