...
- **⚠ SUPERSEDED 2026-08-06 by M15 F3 leg 1 — read before following the apparatus line below.** The sheet is no longer refused at every tier. `resolve.js` now admits **exactly** `readDom` / `readAxTree` / `captureScreenshot` while the sheet's **current** menuType is `bookmarks-overflow` or `bookmark-edit`; `evaluate` / `injectScript` / `openDevTools` stay refused unconditionally. Sheet CONTENT under those menuTypes is agent-readable now, so a step that only needs to *read* it should not be marked operator-performed. Steps that **type into, click inside, or dismiss** a sheet remain operator-performed — no admitted op drives input.
