...
|---|---------|------------------|
| 1 | **Active-precondition probe.** Connect the MCP client; `initialize`; `tools/list`. | `tools/list` **includes the tools this spec drives** (e.g. `openTab`, `enumerateTabs`, `readDom`, `readAxTree`, `captureScreenshot`) — presence-checked, not an exact count (the registry total grows independently). **If any are absent, halt.** |
