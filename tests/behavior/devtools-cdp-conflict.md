...
>
> **Annotation (Flight 7 / leg `harden-ungated-path`, 2026-06-16).** The BLOCKED-AS-WRITTEN state is gated solely on the **missing non-CDP DevTools-open affordance** above — it is **NOT** related to the `:9222` `--remote-allow-origins` Origin that Flight 7 narrows separately. This spec's venue is `dev:automation` (no `--remote-debugging-port`, no `:9222`), so the Origin hardening is irrelevant to it and does not unblock or alter it. Status stays `draft`; the unblock is carried to **F8-eval** (when the non-CDP DevTools affordance lands).
