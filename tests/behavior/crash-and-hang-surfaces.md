...
| 1 | Open a tab via `openTab` to `http://127.0.0.2:{P}/links.html`; then `navigate` it to `http://127.0.0.2:{P}/form.html` (two history entries). Read its `pid`. From the shell: `kill -SEGV <pid>`. Wait up to 5 s (settle: two identical census reads 500 ms apart). Capture the chrome; read the chrome a11y tree; enumerate tabs. | The page area shows the crash panel — heading "This page crashed", the address `…/form.html`, the raw `crashed (<exitCode>)` line, a **Reload** button — never a gray surface. Census: `loadState: "crashed"`, `loadError.name` `crashed`, `pid: null`, `url` `…/form.html`. The strip entry carries the glyph and a "— crashed" suffix. |
...
| 13 | Tail `crash-log.jsonl`: count lines since `{N0}`; parse the new lines. | ≥ 8 new lines (guest SEGV, guest KILL, hang-kill, storm ×2, chrome ×5); every line has exactly the allowed keys; guest lines carry `origin: "http://127.0.0.2:{P}"` — no path, no `links.html`/`form.html`, no title; chrome lines carry `kind: "chrome"` and a `recovery` of `reloaded` (×4) then `paused` (×1). |
...
**Row notes**: Leg-3 smoke found `kill -SEGV` against a SANDBOXED guest renderer unreliable in one session (the chrome view, `sandbox:false`, crashed every time) — if a guest stays `ok` 5 s after SEGV, the Executor sends `kill -ABRT` (still `reason: crashed`) and only then `-KILL` (`killed`, the "closed by the system" wording); the Validator judges the wording against the signal actually used. DRAFT — leg 4 finalises exit codes and wordings after leg 1's
