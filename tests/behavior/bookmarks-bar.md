...
| 12 | Operator: quit the app; with the app down, insert one row with an invalid `url` into the `bookmarks` table beside its valid siblings in the same jar (bookmarks are a real jar-keyed table since M15 Flight 2, not a `documents` blob row — e.g. `sqlite3 <userData>/app.db "INSERT INTO bookmarks (id, jar_id, url, title, icon, position, added_at) VALUES ('bad-row-1', '<jarId>', 'javascript:alert(1)', 'Bad', NULL, 999, 0)"`, using the active jar's real id and a `position` past the existing entries); relaunch. [mixed-frame] — the corrupting action is only possible at the storage layer; the observable is the UI's repair behavior. | The app starts normally (no boot failure); the invalid row is silently dropped (never rendered on the bar), while its valid siblings in that same jar still render, in their prior order. |
