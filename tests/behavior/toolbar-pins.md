...
badge**; unpinning (from the settings Startup & appearance section) **removes the toolbar icon** but leaves its
...
(`toolbarPins.devtools === false`, DD4): a power-user tool, opt-in via right-click / Settings → Startup & appearance,
...
- **Two distinct targets (dual-target spec — keep them straight):**
  ...
  - **Internal guest target** (from `enumerateTabs` → the entry with `url: 'goldfinch://settings'` →
    its `wcId` as `guestWcId`): the settings **Startup & appearance** pin toggles. Read via
...
- **Coordinate-click rule (apparatus rule from the leg-2 spike):** all clicks are coordinate-based —
  ...
  the MCP surface (the toolbar icons, the kebab, and the Startup & appearance pin toggles are all located by
...
- **Focus-anchor rule (apparatus rule from the leg-2 spike):** a cold `Tab`/keyboard sequence from
  ...
  — `click(guestWcId, x, y)` on the Startup & appearance area for the guest toggle, `click(wcId, x, y)` in the
...
- The build includes the `toolbarPins` store key (now `{ media, shields, devtools }`), the icon toolbar +
  pin-apply, the Startup & appearance pin toggles (Media + Shields + **DevTools**), the "Site settings →" rewire, and
...
- **DevTools default UNPINNED (load-bearing for the initial assertions, DD4).** A fresh / upgraded
  ...
  carries `.hidden` in the chrome toolbar and the Settings → Startup & appearance DevTools pin reads
...
- mcp (admin MCP tools — measured via the admin MCP client connected with the admin Bearer header):
  ...
  `wcId`; the settings guest's Startup & appearance pin toggles (`aria-pressed`) via
...
| 2 | Open Settings (take a `captureWindow()` to locate the kebab (⋮), `click(wcId, x, y)` to open it, then `click(wcId, x, y)` on the Settings item — or the identical trusted path `openTab('goldfinch://settings', null, {trusted:true})`); wait for load; call `enumerateTabs` and record the `goldfinch://settings` entry's `wcId` as `guestWcId`. Read the **Startup & appearance** section via `readDom(guestWcId)`/`readAxTree(guestWcId)`/`captureWindow()`. | The Startup & appearance section shows a **pin-icon toggle button** for **Media** and **Shields** (pushpin glyph, `aria-pressed`), both **pinned** (`aria-pressed="true"`, filled). `enumerateTabs` includes the `goldfinch://settings` entry. `[a11y]` |
| 3 | In the settings guest, establish a focus anchor with `click(guestWcId, x, y)` on the Startup & appearance area (located via `captureWindow()`), then move keyboard focus to the **Media** pin-icon toggle (`pressKey(guestWcId, 'Tab')`) and activate it by keyboard (`pressKey(guestWcId, 'Enter')`/`'Space'`) to UNPIN it. Re-read via `readAxTree(guestWcId)`. | The Media pin toggles to unpinned (`aria-pressed="false"`, outline glyph); keyboard-operable. `[a11y]` |
...
| 7 | Re-pin Media from the settings Startup & appearance toggle (back ON): `click(guestWcId, x, y)` (located via `captureWindow()`) or keyboard-activate the Media pin toggle. Re-read `settings.json` (filesystem) and the chrome toolbar (`readDom(wcId)`). | `settings.json` `toolbarPins.media === true`; the Media icon **returns** to the toolbar. |
...
| 9 | Run `npm run a11y` (chrome) and `npm run a11y -- --target=goldfinch://settings`; read both results. | **No NEW** violations vs the pinned `ACCEPTED` baseline — the icon toolbar (chrome) and the Startup & appearance pin toggles (guest) introduce no new WCAG A/AA violations. (The chrome run also exercises the new `devtools-button` audit state — it pins DevTools, un-hides `#toggle-devtools`, and audits the button's static a11y. See the note below.) `[a11y]` |
| 10 | **(DevTools pin — default-unpinned baseline → pin via Settings → Startup & appearance.)** In the settings guest, read the **Startup & appearance** section (`readDom(guestWcId)`/`readAxTree(guestWcId)`/`captureWindow()`) and confirm a **DevTools** pin-icon toggle is present. Establish a focus anchor with `click(guestWcId, x, y)` on the Startup & appearance area, then activate the **DevTools** pin toggle (`click(guestWcId, x, y)` on its glyph, or keyboard: anchor → `Tab` to it → `Enter`/`Space`) to **PIN** it. Re-read `readAxTree(guestWcId)`. | The Startup & appearance section shows a **DevTools** pin-icon toggle (pushpin glyph, `aria-pressed`), initially **unpinned** (`aria-pressed="false"`, outline glyph) — unlike Media/Shields. After activation it toggles to **pinned** (`aria-pressed="true"`, filled); keyboard-operable. `[a11y]` |
...
assertion. Steps 10–13 are the **DevTools** coverage (pin via Settings → Startup & appearance with live button
...
- **The full right-click → "Unpin" context-menu coverage** (for Media, Shields, **and** DevTools) — owned by
  **`page-context-menu.md`**. Since the Leg-5 migration, the toolbar Unpin renders the **in-DOM custom
  ...
  Startup & appearance pin toggle (both paths write `toolbarPins` + broadcast, so the store/toolbar effect is
