...
| 6 | Read `#automation-port.value` via `readDom(guestWcId)` and `automationPort` from the store (filesystem). Focus-anchor `click(guestWcId, …)` on the port field and enter `80` (out of range). Read the Save button's disabled state and the **persisted** `automationPort`; do not synthesize a click on a disabled control. Then enter `{port}` (valid), confirm Save enables, click it, and re-read the **persisted** value. | The pending field shows the stored `automationPort`. With out-of-range input, Save is disabled and the persisted port remains unchanged (filesystem read-back — authoritative); the defensive "Invalid port (1024–65535)" handler remains unit/bridge-covered for callers that bypass UI validation. Valid input enables Save, persists successfully (filesystem read-back matches `{port}`), and shows no error. |
...
| 8 | **Precondition: the toggle is ON** (Step 3 flipped `automationEnabled` from its start value; if Step 3 left it OFF, flip it ON again here so the DD9 gate permits minting). Read the persisted `automationEnabled` (filesystem — record it as `enabledBefore`). In `#automation-jars`, find a persistent jar row with no key (on this dev profile e.g. the legacy `default` jar); read its key-status text + the mint button's `disabled` attribute + the Revoke button's `disabled` attribute via `readDom(guestWcId)` (attribute reads — correct over `readDom`). Locate + `click(guestWcId, …)` its mint button (labeled **Generate key** while `!hasKey`); read `#automation-key-reveal` (present/hidden via `readDom`) + `#automation-key-value` (text via `readDom`); locate + `click` `#automation-key-copy`; re-read the row status + the mint-button label + Revoke `disabled` (all `readDom`). Click the same mint button (now **Rotate key**); confirm a NEW key in `#automation-key-value`. Click **Revoke**; re-read the row + reveal. Read **`automationEnabled` from the store** (filesystem) again. | **DD9 gate:** with the toggle ON, the **mint** button does NOT have `disabled` (key generation permitted). Pre-mint: status `no key`, the **Revoke** button has `disabled` (`readDom`). Generate: `#automation-key-reveal` becomes present/visible with a non-empty one-time key in `#automation-key-value` (`readDom` text); copy succeeds; the row flips to `key set`, the mint button relabels to **Rotate key**, **Revoke** loses `disabled`. **DD3 (Flight 8): minting does NOT change `automationEnabled`** — the post-mint store read **equals `enabledBefore`** (mint stores only `automationKeyHashes`; it has no enable side-effect). The `#automation-enabled` checkbox does NOT flip on a mint (the `settings-changed` broadcast carries no `automationEnabled` change). **Assert the stored value is unchanged.** Rotate: a different key is revealed and the row stays `key set`. Revoke: the row returns to `no key`, **Revoke** regains `disabled`, the reveal is cleared/hidden (all `readDom`); the stored `automationEnabled` is **still** `enabledBefore` (revoke never enables/disables). |
...
always present in the session list, connected name-list, and (as the newest rows) the action log, but
never in the indicator badge (which counts enabled persistent jar keys only). Every viewer/list/log
assertion is relative to that harness baseline, and the staged session is
