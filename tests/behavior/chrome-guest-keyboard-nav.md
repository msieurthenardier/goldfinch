...
| 5 | **Gap (a) — Tab leaves the guest.** Re-focus the guest (`click(G, x, y)` on the input), confirm via `evaluate(G,…)`. Then `pressKey(G, "Tab")`. Observe `evaluate(G,…)`, `evaluate(C,…)` / `readAxTree(C)`, `captureWindow()`. | After Tab, focus **leaves the guest** and lands on a **chrome** control: `evaluate(G,…)` shows `activeElement` is no longer the fixture input (guest released focus), and a chrome element is focused (`evaluate(C,…)` / `readAxTree(C)` shows a focused chrome control); `captureWindow` shows the focus ring in the chrome, not the page. Tab traverses the guest→chrome boundary. |
| 6 | **Gap (c) — chrome Tab-order wraps.** With focus in the chrome (from Step 5), `pressKey(C, "Tab")` repeatedly to walk the chrome's focusable controls past the last one. Observe the focused element via `readAxTree(C)` (focused node) after each press. | Tab **cycles** through the chrome's focusable controls and **wraps** back to the first (or into the guest by design) — focus is **never stranded** on `<body>`/nothing. Each press advances to a real focusable control; the sequence returns to a known start rather than dead-ending. `[a11y]` no focus trap, no lost focus. |
| 7 | **Internal-tab Ctrl+L.** `openTab` to a `goldfinch://` internal page (record **I**); `activateTab(I)`; `click`/focus into it. `pressKey(I, "l", ["control"])`; observe **chrome only** via `evaluate(C,…)` / `readAxTree(C)` + `captureWindow` (do **not** `evaluate(I,…)` — internal sessions refuse observe reads, DD7). | Ctrl+L focuses the address bar on the internal tab too (the intended default — Ctrl+L is a chrome-level accelerator, not a guest feature): `evaluate(C,…)` shows the address input focused; no crash, no misroute; the internal trust boundary is intact. *(If the leg's design intentionally leaves Ctrl+L inert on internal tabs, this step's expectation is updated at that decision — see the flight's Open Questions.)* |
