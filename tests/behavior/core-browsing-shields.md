...
- **This is an ADMIN spec — the admin key is required, a jar key is refused.** The key assertion (Step 5,
  ...
  admin-gated). The guest navigation (Steps 3–4, 6) drives guest WebContentsView tabs via `openTab`/`navigate`, but
...
| 2 | App-launch smoke: call `getChromeTarget()` and `enumerateTabs()`. | `getChromeTarget()` returns the chrome `wcId` (the Goldfinch chrome renderer is up) and `enumerateTabs()` lists **at least one guest** WebContentsView tab — the app started cleanly on the new Electron (no white screen / crash). |
