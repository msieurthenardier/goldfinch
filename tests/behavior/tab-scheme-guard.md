...
Verify that a hostile web page cannot cause Goldfinch to load a dangerous-scheme URL (`file:`, `javascript:`, `data:`) into a guest WebContentsView — through any of the three reachable vectors: the `window.open()` path, the media-panel "open as tab" path, and **in-page self-navigation** (`window.location = 'file://…'`) — while a legitimate `https:` `window.open()` still opens normally. This needs a behavior test rather than a unit test because the threat is a *real page* exercising the real enforcement chain inside the running Electron app: `setWindowOpenHandler` → IPC → `createTab` (renderer gate) **and** the main-process `will-navigate` guard. The unit test (`isSafeTabUrl()`) covers the pure predicate, but only the running app proves the predicate is wired into *both* enforcement points and that a guest WebContentsView never actually navigates to `file://` by any path.
...
- **Active precondition probe** (Step 1 below): confirm `tools/list` includes (presence-checked, not an exact count) the tools this spec drives:
...
|---|---------|------------------|
| 1 | **Active-precondition probe.** Connect the admin MCP client; call `tools/list`; call `getChromeTarget()`; `curl` the trigger page URL. Then `openTab('http://127.0.0.1:PORT/')` (the trigger fixture) and record its `wcId` from `enumerateTabs()`. Wait for it to load. | `tools/list` **includes** (presence-checked, not an exact count) the tools this spec drives: `getChromeTarget`, `openTab`, `enumerateTabs`, `readDom`. `getChromeTarget()` returns a numeric chrome `wcId`. The trigger page returns HTTP 200 and renders in the new tab; `enumerateTabs()` shows the new web-guest `wcId` on the `http://127.0.0.1:PORT/...` URL (the `trigger-page-loaded` marker is visible). Record the chrome `wcId` and the web-guest `wcId`. If any fail, halt — preconditions not met. |
...
| 3 | On the trigger page, click the **`open file:///etc/passwd`** button (locate `#open-file` via `captureWindow()`, then `click(guestWcId, x, y)` — the page's own `window.open('file:///etc/passwd')` fires). Observe via `enumerateTabs()`, `readDom`, and `captureScreenshot`. | No tab renders the contents of `/etc/passwd`; no tab in `enumerateTabs()` has a `file:` URL; no guest WebContentsView ever carries a `file://` URL. (A blocked attempt may leave no new tab, or a blank/homepage tab — but never local file content.) The harness only clicked the page's button and observed. |
...
| 14 | **Subframe dangerous-scheme navigation (Mission 13 Flight 3, leg `nav-guards` / DD3 AC1, `will-frame-navigate`).** On the trigger page, click a button that injects an `<iframe>` and then, from inside that subframe's own script, self-navigates the subframe to a dangerous scheme (e.g. `iframe.contentWindow.location = 'file:///etc/passwd'`, or a same-origin subframe whose own script does `window.location = 'javascript:alert(1)'`) — a **subframe**, not the top frame, so this exercises `will-frame-navigate` specifically (distinct from Step 5's top-frame `will-navigate`). Observe via `readDom(guestWcId)` (the subframe's rendered content/URL) and `captureScreenshot`. | The subframe does NOT navigate to the dangerous URL — it either stays on its prior content or shows a blocked/blank state; no local file content or injected script effect (e.g. a title change) is observable anywhere in the tab. The top frame's own URL (`readDom(guestWcId).url`) is unaffected (still the trigger-page URL). *(The fixture does not yet expose a subframe self-navigation vector as of this leg — add an iframe + subframe-script trigger to `tests/behavior/fixtures/tab-scheme-guard/index.html` as part of running this step, following the existing button + status-element pattern used by Vectors 7–10.)* |
| 15 | **Real cross-scheme redirect (Mission 13 Flight 3, leg `nav-guards` / DD3 AC5, `will-redirect` + the Adaptation Criteria's live-behavior-change check).** Click a trigger-page button that causes the **server** to issue an HTTP redirect (302/303) from the trigger page's own origin to a **different, non-`http(s)` scheme** — e.g. a redirect to a custom/app-handoff scheme such as `intent://` or `myapp://callback` (simulating an OAuth/payment app-handoff), or, as a same-machine substitute, a redirect to `mailto:` — and separately, as the **ordinary-browsing control**, a button that redirects `http://` → `https://` (same-scheme, must keep working). Observe via `enumerateTabs()`/`readDom(guestWcId)`. | **Cross-scheme redirect:** the guest's URL does NOT become the custom-scheme target (`will-redirect`'s guard cancels it, per `isSafeTabUrl`); the tab remains on its pre-redirect URL. Record whether this is a **legitimate** flow that a real user would expect to complete (per DD3's Adaptation Criteria) — if so, this is the divert trigger: scope down the guest redirect guard rather than shipping the regression. **Same-scheme control:** the http→https redirect completes normally and the tab lands on the `https://` URL — confirms `will-redirect`'s guard does not break ordinary redirect-based browsing. *(`python3 -m http.server` cannot itself issue a custom 302 — the FD substitutes a small redirect handler, e.g. a one-off Node `http.createServer` alongside the fixture, or an equivalent CGI/redirect-capable static server, for this step only.)* |
