...
| 6 | Navigate any tab to a page with selectable text, select a short phrase, right-click it, and choose the "Search for …" item. | A tab opens on a `duckduckgo.com` search URL containing the selected phrase — the context-menu entry point uses the same chosen engine. |
