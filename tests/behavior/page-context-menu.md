...
> `returnFocus` element when one exists (keyboard invocations), else the **address bar** — not the
> guest.
...
| 8 | **Keyboard nav + Escape focus-return.** With the menu open (guest right-click), `pressKey(sheetWcId, 'ArrowDown')` / `'ArrowUp'` / `'Home'` / `'End'`; read the sheet roving state between presses. Then `pressKey(sheetWcId, 'Escape')`; read the focused node in `readAxTree(chromeWcId)`. | Arrow/Home/End move the **roving focus** across `role="menuitem"` items (skipping separators/notes); **Escape** closes the menu (gone from pixels) and focus returns to the **chrome** — for a pointer-invoked (right-click) menu there is no captured `returnFocus` element, so focus lands on the **address bar** (`#address`); for a keyboard invocation it returns to the invoking element (Step 9). It is NOT stranded on `<body>` — and (changed from the freeze era) it does NOT land in the guest. `[a11y]` |
