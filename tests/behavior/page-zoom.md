...
| 7 | Identify the `goldfinch://settings` internal tab's wcId (open it via the trusted path). Attempt `setZoom(internalWcId, 1.5)` under the **admin** key. | The attempt is **refused / no-op** via the op's *op-local* internal guard — `getZoom` on the internal tab does not report `1.5`; the refusal is clean, not an opaque error. |
