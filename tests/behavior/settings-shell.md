...
| 3 | Call `readDom(guestWcId)` and `readAxTree(guestWcId)` on the `goldfinch://settings` guest; take a `captureWindow()` screenshot. | The guest renders a **persistent left section-nav** with the 4 links (Startup & appearance, Privacy & Shields, Automation, About) and **4 titled `<section>`s** (`appearance`, `privacy`, `automation`, `about`) each with an `<h2>` + content — the former standalone "On startup" section merged into `appearance` as "Startup & appearance" (M15 F1 Leg 3, DD7). Recognizable as a settings area. |
