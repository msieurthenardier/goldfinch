...
| 3 | **Jar exclusivity.** Clear the input (Ctrl+A + Delete or select-all + type); type `zebraf` (the work-jar marker prefix). | ZERO **history** rows for the marker — the work jar's history never leaks into a default-jar tab's suggestions. Bookmark rows (marked as bookmarks) ARE permitted here (M15 F1 DD11: bookmarks are app-scoped and may surface in any jar) — this run's fixture seeds no bookmark matching `zebraf`, so the observable result is still the empty state ("No matches" note), but the absolute "zero suggestion rows" framing is deliberately narrowed to history-only per DD11, not a coincidence of this fixture. *(the mission's exclusivity clause at the omnibox surface, now history-scoped)* |
