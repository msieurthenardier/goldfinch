...
| 9 | **Omission states — single tab.** Reduce to one tab: right-click the reopened tab and activate **Close other tabs** (reuses Step 5's proven body; leaves exactly the reopened tab, active). Re-read rects. Right-click the SOLE remaining tab. Read the menu items. Dismiss. **Then a two-window sub-case (M09 F10 L3):** open a SECOND window (`evaluate(C, "window.goldfinch.windowNew && window.goldfinch.windowNew()")` or the app's new-window path); poll `enumerateWindows()` until TWO rows. Re-read rects in the first window. Right-click its SOLE tab again and read the menu items. | Items EXACTLY: **Close**, **Duplicate**, **Reopen closed tab** (the stack is non-empty from Step 9's own sweep — reopen present is the positive control proving Step 3's absence was the empty-stack RULE, not a blanket omission) — **NO "Close other tabs"** (only tab) and **NO "Close tabs to the right"** (none to its right). **NO "Move to new window"** (a sole-tab move to a NEW window is a no-op swap — omitted at `isLastTab`, M09 F6) and **NO "Move to window …"** — but the move-window absence here is because there is **NO OTHER window**, NOT because the tab is sole (M09 F10 L3 reversed the sole-tab omission for that item). Separators sit only between populated sections (never leading/trailing). **Two-window sub-case:** with a second window open, the SOLE tab's menu now INCLUDES **exactly one "Move to window …"** item (captioned from the second window's active tab) — a sole tab may consolidate into an existing window, main closing the emptied source — while **"Move to new window" STAYS ABSENT** (still a no-op swap). This is the AC1 presence proof for a sole tab: move-window keys on `!isInternal` alone, move-new-window still on `!isLastTab`. Dismiss via Escape. |
