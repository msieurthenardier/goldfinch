...
- *Unreachable*: the page-context "Search for …" item (sheet outside `AUTOMATABLE_MENU_TYPES`) — its no-engine path is an `[operator]` row, named as such in the spec (Flight 1's lesson).
