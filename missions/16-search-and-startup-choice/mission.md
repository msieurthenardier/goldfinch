...
The welcome page is the first surface in the app that must be **trusted and browsable at once** — it writes preferences through the privileged bridge, yet both of the user's stated escape hatches (type an address, click a bookmark) are exactly what internal tabs disable. It needs a bespoke combination: (1) and (2) must flip, the privileged bridge must stay, and the remaining four are open rulings. Resolving that split is the mission's principal design risk and the reason this is not a one-flight change.
...
- [ ] **Branding.** What the branded page actually looks like. The only existing asset is `build/icon.png`; a first-run surface likely needs more, and that is design work with no precedent in this codebase.
