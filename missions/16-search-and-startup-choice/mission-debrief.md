...
- [ ] **Synchronous local cache write** at `chrome-welcome-set` and `internal-settings-set` so `homePageCache`/`searchEngineCache` never lag their own write; then retire the `{ search }`/`{ home }` override pattern. Squawk-sized or a one-leg flight — decide at the harness flight's planning.
      *— re-verified open 2026-08-27: the `{search}`/`{home}` override is still in welcome-controller.js*
- [x] Squawk 0013 — rewrite CLAUDE.md's welcome-surface paragraph (stale on three counts); add the DOM-contract + "logic behind a review" rule and name the viewless tab record as a reusable primitive in the same pass — *completed 2026-08-26 (PR #172) as the paragraph rewrite only; the DD2 DOM-contract sentence was already present and kept. The "logic behind a review" rule and the viewless-tab-record primitive note were not added and remain open.*
- [x] Squawk 0014 — crew-file apparatus facts, batch 2 — *completed 2026-08-26 (PR #172)*
- [x] Squawk 0015 — welcome spec hygiene — *completed 2026-08-26 (PR #172)*
- [ ] `renderer.js` line budget — rule at the next flight's design that touches it
- [x] Fixed-base-commit timing A/B (M15 F3 `3091 ms / 3558` vs today's `3790`) — one afternoon; closes a three-flight watch item either way
      *— closed by the 2026-08-27 sweep: 3327–3381 ms for 3792 tests vs M16 F2's 3658 ms for 3763; watch item disproven, ticked 2026-08-27 (maintenance sweep)*
- [x] Correct the squawk-0003 claim in the F2 flight debrief (annotated alongside this debrief)
- [x] Methodology items 1–7 above → mission-control skill edits — reviewed one at a time with the operator on 2026-08-26 (mission-control PR #8). Applied, each narrowed to its project-independent kernel: **1** (re-authored rows are new premises — trace before the next run, record outside the Executor-facing cells; `AUTHORING.md` only), **2** (reframed: a HAT is for alignment and may or may not change behavior; scope surfaced is relative to the scope behind it; HAT leg vs HAT flight is the operator's trade-off; `/flight` only), **3** ("State-machine reachability" widened to "Reachability" of any condition an acceptance criterion depends on; container-query advice dropped as a CSS tip), **5.1** (`[CLOSING]` to the Executor before any wrap-up). Dropped as project-specific or already covered: **4** (pivot corollaries), **5.2–5.5** (restart evidence, replacement Executor, `[mixed-frame]` clause, follow-on rows), **6** (debrief re-reads squawk log), **7** (grep-AC guidance — belongs in this project's CLAUDE.md beside its structural-test convention, if anywhere).
