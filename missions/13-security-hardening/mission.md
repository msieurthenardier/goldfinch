...
- [x] Flight 3: **Policy and IPC hardening batch** — permission allowlist inversion, sender validation across chrome-trust channels + `isSafeTabUrl` on `tab-navigate`, `will-frame-navigate`/`will-redirect`/`web-contents-created` guards, vault-capture `isTrusted` guard. *(findings 2, 4, 5, 6 — mechanical, shared risk profile)*
