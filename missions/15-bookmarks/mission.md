...
- **Jar-scoping ruling (REVISED 2026-07-30 — diversion; supersedes the original app-scoped ruling below)**: bookmarks are **jar-owned**. Each jar has its own bookmark list; the bar, the star, and address-bar suggestions all reflect the active tab's jar only, and no bookmark data crosses a jar boundary in either storage or presentation. The bar's on/off toggle remains **app-wide** (a display preference, not jar data). Burner jars are **inert** for bookmarking — no star, no bar — so a bookmark can never be saved into a session that will discard it.
  - *Superseded original (Flight 1 shipped against this)*: ~~the bookmark list is app-scoped (global across cookie jars); it may surface in address-bar suggestions in any jar — a deliberate, presentation-only crossing of the jar boundary.~~ Reversed by operator diversion: jar ownership is required for the roadmap goal of exporting a jar as a unit, and app-scoped bookmarks leak browsing context between jars that exist precisely to separate it.
- **Migration policy (diversion decision)**: existing app-scoped bookmarks are **dropped** on migration (clean slate). Acceptable only because the feature shipped days ago with no accumulated user data; the refactor must not carry a translation path that would imply otherwise.
...
- [ ] **Flight 1 shipped app-scoped bookmarks** (correct against the ruling in force at the time; superseded by the 2026-07-30 jar-scoping diversion). Until Flight 2 lands, bookmarks are shared across all jars, and the active `bookmarks-omnibox` behavior spec still *asserts* that cross-jar visibility at checkpoint 4 — that assertion inverts in Flight 2. Discovered by the operator post-Flight-1; affects the store shape, the broadcast payload, the chrome cache, the bar, the star, the omnibox source, and three active behavior specs.
...
- [x] Flight 2 **(inserted by diversion 2026-07-30)**: **Jar-scoped bookmarks** — move bookmark storage from the app-scoped document to jar ownership (shape chosen with future jar-export in mind); `bookmarks-changed` gains a jar dimension (the `history-changed { jarId }` precedent); the chrome cache, bar, star, and omnibox source all become jar-aware, adding "active tab's jar changed" as a new re-derive trigger; burner-jar inertness; jar-deletion teardown; clean-slate migration. Inverts one asserted step in the active `bookmarks-omnibox` spec and extends the other two specs with a jar dimension. Sequenced BEFORE drag deliberately: drag mutates list order and membership, so building it against the app-scoped model would mean building it twice.
- [ ] Flight 3: **Drag interactions** — reorder within the bar, move between bar and overflow in both directions, drag onto the page to load. Separate flight because it carries the mission's distinct risk cluster (drag across native-surface boundaries, unproven — no precedent in the codebase) and consumes the spike verdict and real bar from Flight 1. Opens with the drop-semantics decision and, if the spike failed, the fallback renegotiation.
...
## Diversion Record — 2026-07-30: bookmarks become jar-scoped

**What changed**: the operator identified an oversight in the original mission framing — bookmarks were specified app-scoped (global across cookie jars), but they should be **owned by the jar**, so that switching to a tab in another jar switches the bookmarks shown, and so a jar can eventually be exported as a self-contained unit. The bar's on/off toggle remains app-wide.

**Why this is a diversion, not a new mission**: the outcome ("keep a personal list of web pages and return to them") is unchanged; only the scoping of that list moves. The mission's flights, criteria, and constraints are amended in place with the superseded framing preserved as commentary, per the mid-execution scope-change protocol — inspection records are snapshots, not living plans.

**Sequencing decision (Flight Director)**: the refactor is inserted as Flight 2 and drag moves to Flight 3, rather than appending the refactor after drag. Rationale: drag mutates list *order* (reorder) and *membership* (bar ↔ overflow), both against whatever "the list" is — so building drag against the app-scoped model guarantees touching it again. The refactor's cost also scales with consumer count, and today's four consumers (star, bar, overflow, omnibox) is the smallest that set will ever be. Finally, the reversal changes a *shipped* constraint and an *asserted* behavior-test step; flipping that once, now, is cleaner than flipping it after drag layers semantics on top.

**Operator decisions captured**: migration is a **clean slate** (existing app-scoped bookmarks are dropped, acceptable because the feature is days old with no accumulated data); burner jars are **inert** for bookmarking (no star, no bar) so a bookmark can never be written into a session designed to discard it.

**Flight 1 is unaffected and stays `completed`.** It was correct against the ruling in force when it was designed and built; the diversion is a forward change, not a defect finding.
