...
**Behavior test bookmarks-bar**: PASS 12/12 (run 2026-07-29-18-23-40; run log committed). Checkpoint 8 FAILED first attempt — overflow menu clipped at the window's right edge with fragmented labels (operator eyewitness ruled out capture artifact); root cause was the left-edge anchor idiom on a far-right trigger collapsing shrink-to-fit width; fixed 5d2d098 (kebab right-aligned idiom, factored as reusable rightAnchorOf + regression pin), app relaunched, checkpoint re-run PASS. Spec graduated draft→active with apparatus notes folded into preconditions.

Notable positives witnessed: all run-time edits (rename, URL change, removal) survived restart with icon distribution intact; corrupt-store repair produced a clean empty list with damage fully contained to the bookmarks row (settings/jars/downloads/session all intact) and re-starring worked immediately; DD6 exact-URL icon gating positively confirmed on THREE independent triggers (MDN redirect → withheld; HN direct match → backfilled; TypeScript revisit → backfilled) with never-visited siblings staying null.

Apparatus/testing lessons for the debrief: (1) overlay sheet is unautomatable at every tier — sheet steps are permanently operator-assisted; (2) readAxTree not readDom is authoritative for live checkbox state; (3) every window.goldfinch.* read is Promise-based and silently yields {} unasted — a false-negative trap; (4) no hover primitive → native tooltips un-assertable; (5) post-relaunch, a stale instance answers 401 rather than refusing, and the continuing window must be identified by tab-set match not lastFocused; (6) Validator recommends formalizing three operator-corroboration evidentiary tiers in AUTHORING.md (temporal/animation null-result, apparatus-gap direct report — should require content-specific wording, frame-occlusion re-observation).

Cosmetic items accumulated for operator disposition at HAT wrap: low-contrast outline star on dark theme; overflow menu's slight right-offset from the chevron; bar item retains focus/pressed highlight after click.

**Behavior test bookmarks-omnibox**: PASS 6/6 (run 2026-07-29-23-13-53; run log committed). Spec graduated draft→active with the step-3 control fixture and apparatus notes folded into preconditions. All DD11 clauses positively witnessed: bookmark-history dedup (single badged row for a URL in both sources), bookmark-only surfacing (never-visited bookmark appears), badge is bookmark-specific (history-only control row unbadged), cross-jar app-scoped bookmarks WITHOUT history leakage (the mission's jar-boundary ruling, negatively asserted), keyboard selection preserving jar identity, and post-removal demotion to a plain history row (title reverting from bookmark title to live page title — a sharp anti-ghost signal the Validator recommends promoting to a named clause). Validator pre-run review caught a genuine fixture gap (no history-only term named) which the Executor closed empirically before executing — best process moment of the run. Harness security flag on Executor startup API probing assessed FALSE POSITIVE (sanctioned dev key, app under test, method-name discovery).
...
**2026-07-29 — HAT fix: suggestions-sheet star badge undersized / not row-aligned.** Operator follow-up on the just-shipped star glyph (previous entry, 0b4d8e9): the star rendered small and sat as a corner chip rather than reading as a full-height indicator aligned with the row. Request: scale the star to span the row's full two-line content height (`.sg-primary` + `.sg-secondary`) and center it vertically top-to-bottom.

Approach chosen: **flex stretch, not a hardcoded pixel size** (the guidance's preferred option — sized off the row's own height so it stays correct if line-heights change, rather than a fixed size matched to a one-time measurement). `.sg-option` (`menu-overlay.css`) flips from a column flex (primary stacked over secondary, badge absolutely positioned in the top-right corner) to a ROW flex: the primary/secondary pair moves into a new `.sg-text` span (`menu-overlay.js`'s `renderSuggestions`) that reproduces the old column stack exactly, and `.sg-badge` — when a row is a bookmark — becomes a real flex sibling instead of an absolutely-positioned chip. Flexbox's default `align-items: stretch` (stated explicitly on `.sg-badge` too, for clarity) sizes the badge to `.sg-option`'s content-box height — i.e. exactly the two-line text height, padding excluded, which is precisely "the row's full content height" as requested. The star SVG itself (`.sg-badge-star`, targeting `buildBookmarkStarIcon`'s class) gets `height: 100%; width: auto; aspect-ratio: 1 / 1`, overriding the builder's `width="11"/height="11"` presentation-attribute defaults via ordinary CSS cascade — no change to `bookmark-star-icon.js` itself (pure consumer-side CSS/DOM change, builder contract untouched). `.sg-text` needed `min-width: 0` (overriding the flex item default `min-width: auto`) so the nowrap primary/secondary spans keep shrinking/ellipsizing instead of being forced wide by their intrinsic content width — preserving truncation and the row-width-vs-sheet overflow fix from the immediately preceding commit (`.sg-option`'s `box-sizing: border-box` untouched).
