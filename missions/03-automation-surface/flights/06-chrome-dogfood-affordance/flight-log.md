...
### 2026-06-16 — FLIGHT LANDED [COMPLETE:flight]
All 8 legs completed. The live block (legs 2 spike → 4 migrate → 7 verify → 8 HAT) ran in an operator session on the clean `dev:automation` admin surface. **HAT passed** (operator sign-off) after fixing three inline gaps (SC10 audit per-action context + typeText redaction; port Save dirty-state; "(unsaved — applies on Save)" note; disabled-button styling). Final independent review of the live-block changes: **[HANDOFF:confirmed]** — `npm test` 650/650, typecheck + lint green; typeText redaction + XSS-safe rendering verified. Committed; flight `landed`; checked off in mission.md (scoped reframe annotated); **PR #44 marked ready for review**. Debrief is the separate `/flight-debrief` step (transitions the flight to `completed`).

**SC11 part 1 (scoped) delivered**: the chrome-driving affordance exists and a representative subset dogfoods on the surface. **Remaining (follow-on + Flight 7)**: the bulk Group-B migration, the `a11y-audit.mjs` rewrite, and retiring the ungated `:9222`/`dev:debug` path. F5 fast-follows (activity-log paging/clear/retention) still open; F6 added the audit `detail` field but not paging.

## Leg Progress

### Leg 07: verify-integration — landed (2026-06-16) — PASS
FD-driven focused live pass (operator-elected) of the 3 migrated specs on the **admin MCP surface** (admin Bearer key over the clean `dev:automation` instance, `127.0.0.1:49707`, no `:9222`):
- **tab-keyboard-operability — PASS**: arrow/Home/End move focus+selection (one `aria-selected`, address changes); `Delete` closes focused tab (4→3) → focus to sibling; 1 tablist/3 tab/1 selected.
- **kebab-menu — PASS**: trusted click opens menu = exactly [Settings, Exit]; `Escape` closes + restores trigger focus.
- **settings-shell — PASS**: kebab→Settings opens the admin-enumerable `goldfinch://settings` guest (wcId 6); shell = "Settings — Goldfinch", 6 sections/6 h2/962 AX nodes/6 nav links; chrome chip shows the internal identity.
- **Gates**: `npm test` 630/630, typecheck + lint clean. **dev:debug regression clear**: `package.json` `dev:debug` unchanged from main; un-migrated Group-B specs still on `:9222` (untouched; retirement = Flight 7).
- Run logs committed at `tests/behavior/{tab-keyboard-operability,kebab-menu,settings-shell}/runs/2026-06-16-08-04-29.md`; evidence ephemeral (never committed).
- Deferred-to-HAT (focused pass): tab-keyboard Steps 6/8, kebab keyboard-open + focus-ring delta, settings-shell nav-lock + chip-popup. One coordinate-tuning artifact: Settings menu item needed x=1300 (1265 missed) — normal blind-click tuning.

### Leg 04: migrate-subset-specs — landed (2026-06-15)

**Status**: landed

**Changes Made**:
- `tests/behavior/tab-keyboard-operability.md` — Preconditions completely rewritten: `npm run dev:debug`/`:9222`/Playwright-MCP/`Accessibility.getFullAXTree`/`document.activeElement`/CDP `Input.dispatchKeyEvent` → `npm run dev:automation` + admin MCP client (SDK `StreamableHTTPClientTransport`, Bearer `adminKey`); Step 1 probe: `curl :9222/json` → `tools/list` shows 17 tools incl. `getChromeTarget`; `getChromeTarget()` returns numeric chrome `wcId`; Observables: CDP a11y tree → `readAxTree(wcId)`, DOM reads → `readDom(wcId)`, screenshots → `captureWindow()`; Steps 3–8: `pressKey(wcId, …)` for all key events, `click(wcId, x, y)` for coordinate clicks (Step 8's `#address` click → coordinate via `captureWindow()` ≈ (400,63)), `readAxTree(wcId)` for all a11y/focus assertions. Added focus-anchor rule (Step 3: `click` before keyboard-only sequence) and coordinate-click rule (Preconditions). `chrome-devtools` MCP disqualification warning preserved (wording updated to avoid literal `:9222` in grep-matchable form). Step count: 8 checkpoints unchanged.
- `tests/behavior/kebab-menu.md` — Preconditions rewritten: `npm run dev:debug`/`:9222`/`scripts/cdp-driver.mjs`/Playwright-MCP → `npm run dev:automation` + admin MCP client; `getChromeTarget()` → chrome `wcId`; Step 1 probe: `curl :9222/json` or `node scripts/cdp-driver.mjs eval` → `tools/list` 17 tools + `getChromeTarget()` result; Observables: CDP attach/`getBoundingClientRect`/`document.activeElement` → `readAxTree(wcId)`/`readDom(wcId)`/`captureWindow()`; all 12 steps: trusted clicks → `click(wcId, x, y)` via `captureWindow()` coordinates; trusted keys → `pressKey(wcId, name)`; `document.activeElement` → focused node in `readAxTree(wcId)`. Added focus-anchor rule (keyboard sequences) and coordinate-click rule. `chrome-devtools` MCP warning preserved. Step count: 12 checkpoints unchanged.
- `tests/behavior/settings-shell.md` — Preconditions rewritten: `npm run dev:debug`/`:9222`/`scripts/cdp-driver.mjs` → `npm run dev:automation` + admin MCP client with `adminKey`; explicit two-distinct-targets framing: chrome `wcId` (from `getChromeTarget()`) vs. internal guest `guestWcId` (from `enumerateTabs`); noted admin engine `allowInternal` makes the internal tab enumerable/drivable; Step 1 probe: `cdp-driver eval '1+1'` → `tools/list` 17 tools + `getChromeTarget()` wcId; Step 3 guest DOM: CDP attach → `readDom(guestWcId)`/`readAxTree(guestWcId)`; Step 4 keyboard: `Tab`/`Enter` → `pressKey(guestWcId, …)` with focus-anchor click; Steps 7–8 chrome chip: `readDom(wcId)`/`readAxTree(wcId)` on chrome `wcId`; Step 9 chip click: coordinate `click(wcId, x, y)` via `captureWindow()`; Step 11 address-bar type: `typeText(wcId, url)` + `pressKey(wcId, 'Enter')`. `chrome-devtools` MCP warning preserved. Step count: 12 checkpoints unchanged. Admin-only requirement noted in Preconditions.
- `tests/behavior/mcp-jar-scoping.md` (line ~81) — Updated the "openTab jar-targeting (v1 limitation)" note in Out of Scope: rewritten to reflect that jar-targeted `openTab` is implemented (leg 3 / DD3) — a jar key's new tab now lands in its own jar or is refused (`out-of-jar` / `unknown-jar`); the old "cannot target the jar" caveat no longer applies. Kept informational (not a new assertion).

**Verification**:
- `grep -n "9222\|cdp-driver\|dev:debug\|remote-debugging" tests/behavior/tab-keyboard-operability.md tests/behavior/kebab-menu.md tests/behavior/settings-shell.md` → **no output** (AC6 passed).
- `npm test`: **630 pass / 0 fail**. `npm run typecheck`: clean. `npm run lint`: clean. (No source touched — AC7 passed.)

**Notes**:
- The disqualification warning "not the `:9222` CDP path" was present in all three specs. Rewording was required to avoid the literal `:9222` pattern matching the AC6 grep while still communicating the same intent: changed to "CDP attach path — `npm run dev:automation` does not expose a DevTools port; only the admin MCP surface is used."
- `settings-shell` required the most structural change: the single CDP target (`index.html` renderer) becomes two distinct MCP targets (chrome `wcId` + guest `guestWcId`). The dual-target framing is now explicit in Preconditions with a keep-them-straight note.
- `tab-keyboard-operability` Step 8's `#address` CSS-selector click → coordinate `click(wcId, 400, 63)` — the spike-confirmed ≈ (400, 63) coordinate for the omnibox at 1400×900 is cited in Preconditions with the caveat that the `captureWindow()` screenshot is the source of truth for exact coordinates.
- All three specs carry both leg-2-derived apparatus rules: (a) focus anchor before keyboard sequences; (b) coordinate-based clicks via `captureWindow()` screenshot.
...
- Legs 1, 3, 5, 6 → **completed**, checked off in flight.md, and committed (`3ad20ea`; code + leg artifacts + flight log; the unrelated untracked `src/renderer/assets/gf_01*.png` were deliberately NOT staged). **Draft PR #44** opened (base `main`) with the 4 legs checked, the live legs (2/4/7/8) unchecked. Flight stays **in-flight**.
...
### HAT finding (leg 8, 2026-06-16): audit activity log lacks per-action context — fixing inline
**Context**: During the guided HAT, driving the live browser then opening Settings → automation activity, the operator observed the activity log records the **operation** (`openTab`, `navigate`, `click`, …) and the target `wcId`, but **not the action's context/"where"** — e.g. the URL a navigate/open went to, the coordinates of a click, the key pressed. Without the context, auditing what an automation session actually did is impractical. A genuine SC10 (auditability) gap, surfaced by dogfooding the audit viewer itself.
**Decision (operator)**: **fix inline now** (HAT pattern) rather than defer. Add a per-op `detail` to the audit record (`audit-log.js`), derived from the tool args at the single record call site (`mcp-server.js`), and render it in the activity viewer (`settings.js`). **typeText is redacted to length-only** (`text(N chars)`) — operator decision — so a typed secret (e.g. a password) is never written to the local audit log; URLs / coordinates / key names are logged verbatim (high audit value, low sensitivity). Re-verify live in the HAT before sign-off.

### Leg 2 chrome-drive-spike (DD2 apparatus premise) — RESOLVE, both axes pass (2026-06-15)
**Context**: The migrations (leg 4) assert against the chrome `wcId` via the admin MCP surface; DD2 required verifying both axes live before locking them. A `dev:debug`+`--automation-dev` instance was already running (MCP on :49152, **with the `:9222` CDP confound**). **Operator approved restarting clean**: the FD killed it and relaunched plain `dev:automation` (`GOLDFINCH_AUTOMATION_DEV_MINT=1 GOLDFINCH_AUTOMATION_ADMIN=1 GOLDFINCH_MCP_PORT=49707`, **no `--remote-debugging-port`**) so the `readAxTree`-on-chrome attach is tested confound-free.
**Apparatus**: an admin MCP client (MCP SDK `StreamableHTTPClientTransport`, `Authorization: Bearer <minted adminKey>`) on `127.0.0.1:49707`, driven by an ephemeral FD harness (written to the goldfinch dir, run, **deleted** — never committed). Evidence at `/tmp/behavior-tests/goldfinch/chrome-drive-spike/2026-06-15-20-52-03/`.
**Verdict — RESOLVE (no divert, no engine fix)**:
- `getChromeTarget` → `{wcId:1, kind:'chrome', url:.../index.html}` (admin; 17 tools listed).
- **Observe**: `readDom` = chrome shell (title "Goldfinch", 11.9KB, shell markers present); `readAxTree` = **335 AXNodes, confound-free attach on the chrome target** (resolves the mission's chief open unknown — a11y-tree read on chrome works without `:9222`); `captureWindow` = 82KB PNG compositing chrome + the live Google guest (visually confirmed: tab strip, toolbar, omnibox).
- **Act**: `click(400,63)` focused the omnibox `textbox`; `typeText` mutated its value (`…/spikeprobe`) — real handler fired; `pressKey Tab` (post-anchor) moved focus to the next control (audit indicator) — native focus traversal works.
**Impact / migration note for leg 4**: chrome trusted input AND a11y/DOM reads are fully usable — migrated specs may assert via the a11y tree OR DOM-shape, and may drive trusted click/type/key. One apparatus rule for the migrated specs: **establish a focus anchor (a `click`) before keyboard-only sequences** — a cold `Tab` from the bare document does not relocate the AX focus marker (normal browser behavior, not an engine defect; the initial coordinate-free Tab probe showed no movement for exactly this reason). Bonus observation: the SC10 admin-session indicator renders live and is labeled "1 automation session connected: admin".
...
### Leg 4 design review — SKIPPED (FD decision; spec-authoring, apparatus empirically proven)
`migrate-subset-specs` rewrites 3 specs' apparatus (CDP-`:9222` → admin MCP surface) keeping step semantics. The risky part — does the MCP surface actually drive/read the chrome — was **just proven live by the leg-2 spike** (both axes pass), and the leg carries an explicit CDP→MCP mapping table grounded in that run. A design-review spawn would mostly re-check that mapping, which the spike already validated empirically. Per FD discretion on review depth, the separate cycle is skipped; the implementing Developer self-verifies (no stray `9222`/`cdp-driver`/`dev:debug` in the 3 specs; semantics preserved). The **live runs in leg 7 are the real validation**.
