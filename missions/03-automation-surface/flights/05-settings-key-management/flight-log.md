...
**Part 2 — live behavior-test runs (DONE, 2026-06-15, FD-driven with the operator at the GUI):**
- **`settings-automation` — PASS** (12/13; 1 deferred to the mcp-jar-scoping admin run). Launch: combined `:9222` CDP + `--automation-dev` MCP, port pinned 49707. Drove the settings guest via CDP (`Runtime.evaluate`/trusted click). Verified live: section renders; enable toggle write-through (true→false→true); address `http://127.0.0.1:49707/mcp` + status `Connected — listening on 127.0.0.1:49707`; copy; port validation (`80`→`Invalid port (1024–65535)`, not persisted; `49707`→Saved); find-free-port→`49152` + `(takes effect on next launch)` note; per-jar Generate(43-char key, show-once)→Rotate(new key)→Revoke(cleared) with row state + Revoke-disabled transitions; show-once NOT refetchable after reload (hash persists); admin block hidden (no env gate). Live MCP session lit the chrome indicator (`1 automation session connected: Personal`) + the audit viewer (`jar Personal connected since …`, action log `enumerateTabs personal ok`). SC9 + SC8-toggle + SC10-visible-half confirmed live. Run log: `tests/behavior/settings-automation/runs/2026-06-15-12-40-28.md`. One known limitation recorded (ungraceful-disconnect session lingering — see Anomalies).
- **`mcp-jar-scoping` — PASS (flips the F4 `partial`).** Two relaunches (`dev:automation` + mint; then + `GOLDFINCH_AUTOMATION_ADMIN=1`), operator re-staged the multi-jar tab set each time via the new jars UI (the apparatus F4 lacked). Jar key confined to its jar (enumerate = default only; out-of-jar `wcId 3/4/5` refused; internal `goldfinch://settings` `wcId 6` absolutely refused; burner not drivable). Admin (env-gated key minted) sees all 5 tabs across every jar incl. internal, and drives cross-jar (`wcId 3` personal) + the internal settings guest (`wcId 6`) — the admin tier's sole authorized relaxation. Run log: `tests/behavior/mcp-jar-scoping/runs/2026-06-15-13-12-09.md`.

**Verdict**: leg 6 acceptance met. SC9 + SC10-visible-half + SC8-toggle behavior-test-backed live; the F4 jar-scoping partial is flipped to pass. One known limitation (ungraceful-disconnect indicator staleness) carried as a fast-follow.
