...
- [ ] **HE1 Generalized affordance + guidance.** On `goldfinch://vault` →
      ...
      only.
- [ ] **HE2 Pick + generalized refusal.** A non-export file → the refusal reads
      "…isn't a recognized browser password export." (browser-generic, not
      "Chrome"). The real Edge export → "N logins found" (+ "M rows can't be
      imported" if any).
- [ ] **HE3 Import lands.** Into an empty jar (Merge), the native confirm names
...
- [ ] **HE4 Re-import dedupes.** Same Edge file, same jar (Merge) → "0 imported,
      N already present"; item count unchanged.

**Restore-flow spot-check (CP1 — leg 1 didn't break restore):**
- [ ] **HR1 Export modal.** Settings → Import / Export → Export… opens, offers
      ...
      `setNotice` handoff).
- [ ] **HR2 Restore flow.** Import a vault bundle: pick → the secret sheet → the
...
- [ ] **HR3 Resume affordance.** With a restore mapping modal open, trigger a
...
- [ ] All acceptance criteria verified (or dispositioned)
- [ ] Update flight-log.md with leg progress entry
- [ ] Set this leg's status to `completed`
- [ ] Check off this leg in flight.md
- [ ] Final leg of flight: flight.md status `landed`, check off flight in mission.md
- [ ] Commit
