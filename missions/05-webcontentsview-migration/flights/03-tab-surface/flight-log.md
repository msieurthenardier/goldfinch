...
### Leg 5 HAT — inline fix: kebab/container menus occluded on internal tabs (regression from Leg 3)

**Date:** 2026-06-26 · inline HAT fix on `flight/03-tab-surface`, NOT committed.

HAT regression (from Leg 3): kebab/container menus occluded on internal tabs because internal views are now opaque but the freeze-frame was web-only + `capture-active-guest` refused internal. Leg 3 made internal `goldfinch://` tabs opaque WebContentsViews (they now occlude the HTML chrome menus exactly like web tabs), but the Leg-02b freeze-frame helper was gated web-only (`t.trusted` early-return), so internal tabs never froze → the menu rendered behind the opaque internal view. Compounding it, the defense-in-depth `isInternalContents` guard I added to `capture-active-guest` in Leg 3 blocked capturing the internal page for the still — the "internal never freezes" premise that guard rested on was invalidated by Leg 3's own change.

**Fix:** `freezeGuest`/`unfreezeGuest` now operate on the active view's `wcId` (web or internal) instead of gating on `t.trusted` + `visibleWebTabWcId`; `capture-active-guest` captures internal for the still (safe — the ONLY caller is chrome's own freeze helper, exposed solely on chrome-preload, capturing a page the chrome already displays in its own `#webviews`; not an automation op, not a leak). Site-info on internal tabs also benefits (same helper).

- **main.js `capture-active-guest`:** removed `if (isInternalContents(wc)) return null;`; rewrote the comment block to explain why internal capture is now required and why it's safe. `isInternalContents` is still imported and used elsewhere in main.js (left in place).
- **renderer.js `freezeGuest`:** guard changed from `!t || t.trusted || t.wcId == null || visibleWebTabWcId == null` to `!t || t.wcId == null`; hide call changed from `tabHide(visibleWebTabWcId)` to `tabHide(t.wcId)`; JSDoc updated ("active web guest" → "active guest (web or internal)").
- **renderer.js `unfreezeGuest`:** restore block changed from `if (t && !t.trusted && t.wcId != null) { tabSetActive(...); visibleWebTabWcId = t.wcId; }` to `if (t && t.wcId != null) { tabSetActive(...); if (!t.trusted) visibleWebTabWcId = t.wcId; }` — always re-show the active view; `visibleWebTabWcId` bookkeeping stays web-only.
- **Unchanged:** the `visibleWebTabWcId` set/clear logic in `createTab`/`activateTab`/`closeTab` stays web-only; the freeze now uses `t.wcId` directly so it's independent of that bookkeeping.

**Gates:** `npm run typecheck` → 0; `npm run lint` → 0; `npm test` → **951/951 pass**. Operator confirmed kebab/container now work on internal tabs.

**Polish:** eliminated a one-frame white flash on freeze (decode the still before hiding the live view; dark `#webviews` background as belt-and-suspenders) — most visible on light internal pages.
- **renderer.js `freezeGuest`:** after `captureActiveGuest()` resolves and before setting the bg / hiding the live view, decode the still (`new Image(); img.src = dataURL; await img.decode()`, in a try/catch) so it paints in the same frame the live view is hidden — closes the gap where the PNG hadn't decoded yet and `#webviews`'s default background flashed through. Added a second `stillOpen()` liveness re-check after the decode await (the popup could close during the extra await).
- **styles.css `#webviews`:** `background` changed from `#fff` to `var(--bg)` (`#1e1f25`, matching the chrome shell / chrome WebContentsView background) so any residual gap frame is dark-on-dark (invisible) rather than white. Only ever visible during the freeze gap; the opaque active view covers `#webviews` normally and the still covers it during freeze.

**Gates (polish):** `npm run typecheck` → 0; `npm run lint` → 0; `npm test` → **951/951 pass**. Operator to restart and re-confirm no flash on Settings/Downloads menu open.

**KNOWN ISSUE (logged, operator-accepted — minor, cosmetic).** After both fixes (decode-before-hide + dark `#webviews` background, which removed the literal `#fff`), a **tiny blip still flashes** when opening the kebab/container menu on an internal (Settings/Downloads) tab. The CSS-level white is gone, so the residual blip is attributed to a **WSLg native-view compositing artifact** in the live-view-hide → still-show handoff (the same class of WSLg rendering quirk this mission has hit before — captureWindow distortion, transparent-overlay→black). It does NOT affect function (menus open/work/dismiss correctly) and is internal-tab-only. **Operator decision (2026-06-26): log it and move on** — not a flight blocker. Candidate for a later polish pass / macOS re-check (native-view compositing differs there). Carried as a minor known issue, not a regression of pre-migration behavior (pre-migration internal tabs were `<webview>`s that never froze, so this menu-over-internal freeze path is new surface).

**HAT regression (Leg-3 class):** internal views didn't resize with the window/panel because `sendActiveBounds` had a web-only `!t.trusted` guard; generalized to all active views (web + internal). Self-healed on tab-switch before the fix because `tabSetActive` re-bounds.
- **renderer.js `sendActiveBounds` (~line 1043):** dropped the `!t.trusted` exclusion → `if (t && t.wcId != null)`. Internal tabs are WebContentsViews now and must get bounds on resize (main resize → `trigger-send-bounds` → `onTriggerSendBounds` → `sendActiveBounds`) and panel-toggle (ResizeObserver → `sendActiveBounds`) like web tabs; previously they were stranded at stale bounds until a `tabSetActive`. `measureWebviewsSlotWithInsetDIP` returns the full slot for internal (find is web-only → inset 0); the `guestFrozen` early-return at the top is unchanged.
- **Other `!t.trusted` guards deliberately untouched** (verified correct internal-exclusions): address-bar loadURL, goBack/goForward/reload, find — those operations genuinely don't apply to internal pages. Only the geometry path was wrong.

**Gates (resize fix):** `npm run typecheck` → 0; `npm run lint` → 0; `npm test` → **951/951 pass**. Operator confirmed: **window/panel resize on internal pages now tracks correctly.**

**KNOWN ISSUE (logged, operator-accepted — environmental, suspected WSLg).** On maximize, the **whole window** only reaches the bottom ~2/3 of the screen (a gap at the top shows the desktop behind); the chrome renders fully *within* that short window. This is window-frame placement, not our view geometry: maximize is a standard `mainWindow.maximize()` (window-toggle-maximize IPC → `main.js:1330`) with no `maxWidth`/`maxHeight` constraints (only `minWidth:900`/`minHeight:600`), so the **WSLg/Weston compositor is sizing/positioning the maximized window short**. Consistent with the earlier mission note "maximize doesn't maximize vertically" on WSLg, and the operator suspects a stale compositor state (will retest after a machine restart). **Operator decision (2026-06-26): log it and move on** — not a flight blocker; would behave correctly on macOS/Windows where `maximize()` is honored. *(Latent code-hardening opportunity, separate: the `maximize`/`unmaximize` handlers at `main.js:464-471` send `trigger-send-bounds` but do NOT call `chromeView.setBounds` — they rely on a `resize` event also firing; adding an explicit chrome-view setBounds there would be belt-and-suspenders, but it does not address this window-frame-placement symptom.)*
