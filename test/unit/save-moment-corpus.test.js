...
//   'negative-gesture'    `{ todo: true }` test, assertNoOffer (throws — see
//                          that stub's own docs). DEFERRED to Leg 5.
//   'known-unsolved'      `{ todo: true }` test, assertDetectsEntry. The gated
//                          set may legitimately be EMPTY at this leg's landing
//                          — that is expected, not a failure; Leg 5 populates
//                          it by PROMOTING entries here (an edit to
//                          manifest.js's `tier` field, never a rewrite of this
//                          harness).
//   'gated'                ordinary test, assertDetectsEntry. Zero entries today.
//
...
const KNOWN_ASSERTS = new Set(['detects', 'no-detect', 'offers', 'no-offer']);
