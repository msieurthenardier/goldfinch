...
test('AC7: captureWindow gates its sheet layer on the SAME menuType allowlist the resolver uses', () => {
  ...
  assert.ok(/grabRec\.sheet\s*&&\s*grabRec\.sheet\.isVisible\(\)/.test(masked),
  ...
  for (const menuType of AUTOMATABLE_MENU_TYPES) {
    assert.ok(!masked.includes("'" + menuType + "'"),
  ...
  assert.ok(/registry\.sheetMenuFor\(/.test(masked),
    'the sheet layer must consult registry.sheetMenuFor');
...
test('AC7: the sheet layer\'s post-await TOCTOU re-check re-evaluates the menuType, not only isVisible()', () => {
  ...
  assert.ok(/sheetMenuAdmitted|AUTOMATABLE_MENU_TYPES|sheetMenuFor/.test(condition),
...
test('AC8: each one-time-secret card clears its own textContent INSIDE an onClose() body', () => {
  ...
  for (const scrub of scrubs) {
    ...
    assert.ok(body !== null,
      '`' + scrub + '` must sit inside an onClose() body — a scrub that runs anywhere else ' +
...
test('AC8: onInit\'s pre-scrub early-return set is ENUMERATED — a new early return before the scrub fails here', () => {
  ...
  assert.ok(/typeof menuType !== 'string' \|\| typeof token !== 'number'/.test(preScrub),
    'return 1: the menuType/token shape guard');
  assert.ok(/if \(!modelShapeOk\) return;/.test(preScrub), 'return 2: the model-shape guard');
  assert.ok(/template === 'downloads' && menuController\.current === downloadsEntry/.test(preScrub),
...
test('AC8/DD1f: the sheet registers the EAGER close/reset scrub, and it runs silence() then closeAll()', () => {
  ...
  assert.ok(!/document\.body\.(removeAttribute|dataset)/.test(handler),
...
test('DD1f: main sends the close/reset on the close path, and the preload exposes it', () => {
  ...
  assert.ok(/function deliverCloseReset\(\)/.test(manager),
  ...
  assert.ok(/deliverCloseReset\(\)/.test(body),
    'closeMenuOverlay must emit the eager scrub — the whole of DD1f');

  const preload = maskComments(
    fs.readFileSync(path.join(SRC, 'preload', 'menu-overlay-preload.js'), 'utf8'));
  assert.ok(/onCloseReset:/.test(preload) && /'menu-overlay:close'/.test(preload),
...
test('AC9: sheetMenuFor is wired at BOTH createEngine sites (silent fallback ⇒ grep-pinned)', () => {
  ...
  for (const site of sites) {
    ...
    assert.ok(/sheetMenuFor:\s*\(\w+\)\s*=>\s*registry\.sheetMenuFor\(\w+\)/.test(masked),
      site.join('/') + ' must inject sheetMenuFor — absent, the sheet gate\'s menuType half ' +
...
test('AC9: engine.js threads sheetMenuFor onto deps by the conditional-spread idiom, and exactly three ops opt in', () => {
  ...
  assert.ok(/\.\.\.\(typeof sheetMenuFor === 'function' \? \{ sheetMenuFor \} : \{\}\)/.test(masked),
