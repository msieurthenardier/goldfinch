...
test('AC3: the adopt handler resolves TARGET from the sender and SOURCE from the payload, refusals discriminated', () => {
  ...
  assert.ok(
    body.includes('validateMoveTabPayload(payload)'),
    'the payload is shape-validated before any resolve on it'
  );
...
test('AC3/DD2: the provenance gate precedes the core, allowSoleTab is TRUE, and success CONSUMES the registration', () => {
  ...
  assert.notEqual(
    core,
    -1,
    'the core call passes allowSoleTab TRUE (leg DD5 — a sole-tab drag consolidates and the emptied source closes)'
  );
  assert.ok(gate < core, 'the gate runs BEFORE the move core — a forged payload never reaches it');
  assert.ok(
    body.includes('source.dragWcId = null'),
    'a successful adopt consumes the registration (DD2 refinement: one drag = one drop)'
  );
  ...
  const ungated = real.replace("  if (source.dragWcId !== p.wcId) return { ok: false, reason: 'not-dragging' };\n", '');
...
test('AC4: tab-drag-started verifies the SENDER owns the wcId before recording it', () => {
  ...
  assert.notEqual(
    verify,
    -1,
    'the sender-owns-wcId verification is present — the payload does not get to name a tab the sender does not own'
  );
...
test('AC4: tab-drag-ended clears on the grace timer, never synchronously', () => {
  ...
  assert.notEqual(
    timer,
    -1,
    'the clear is deferred — the adopt invoke rides a DIFFERENT pipe, and an immediate clear races it into not-dragging'
  );
  ...
  const immediate = real.replace(
    /dragEndClearTimers\.set\(\s*rec,\s*setTimeout\(\(\) => \{\s*dragEndClearTimers\.delete\(rec\);\s*rec\.dragWcId = null;\s*\}, DRAG_END_GRACE_MS\)\s*\);/,
    'rec.dragWcId = null;'
  );
...
test('AC4: the renderer bookends — dragstart declares, dragend ends BEFORE the null-session early return', () => {
  ...
  // Wrap-insensitive: Prettier splits the one-line `e.preventDefault(); return;`
  // guard onto two lines.
  const gate = dsBody.search(/e\.preventDefault\(\);\s*return;/);
  ...
  assert.ok(
    gate !== -1 && gate < declare,
    'declared only PAST the null-wcId refusal gate — a null wcId is never registered'
  );
  ...
  assert.ok(
    early !== -1 && ended < early,
    'the bookend precedes the null-dnd early return — a defensively-canceled session still ends its registration'
  );
});
...
test('AC1: dragover accepts UNCONDITIONALLY for the MIME — preventDefault/dropEffect before any dnd gate', () => {
  ...
  assert.ok(
    mime !== -1 && accept !== -1 && effect !== -1 && gate !== -1,
    'the dragover shape moved — re-anchor this pin'
  );
  assert.ok(mime < accept, 'the MIME guard runs first — non-tab drags are never accepted');
  assert.ok(
    accept < effect && effect < gate,
...
test('AC2: the cross-window drop branch — dropHandled first, own-tab guard, adopt invoke, target-side announce', () => {
  ...
  assert.ok(
    body.indexOf('dnd.dropHandled = true') !== -1 &&
      body.indexOf('dnd.dropHandled = true') < body.indexOf('JSON.parse'),
  ...
  assert.notEqual(
    guard,
    -1,
    'the null-dnd-own-tab guard is present — a mid-drag-canceled same-window release is a silent no-op'
  );
  ...
  assert.ok(
    guard < invoke,
    'the guard runs BEFORE the invoke — main is never asked to refuse the canceled-drag corner'
  );
  ...
  assert.equal(
    anchoredBody(unguarded, "els.tabs.addEventListener('drop'").includes(
...
test('AC5a: requestTearOff suppresses EXACTLY the adopted-elsewhere signature — both directions', () => {
  ...
  assert.equal(
    anchoredBody(over, 'function requestTearOff(').includes(SUPPRESS),
    false,
    'mutated → the exact-signature pin FAILS'
  );
  ...
  assert.equal(
    anchoredBody(none, 'function requestTearOff(').includes('!tabs.has(tabId)'),
    false,
    'mutated → the suppression pin FAILS'
  );
});
...
// Wrap-insensitive: Prettier expands the one-line single-statement-per-line if-body
// (`{ clearDragVisuals(); dnd = null; }`) onto three lines.
const SILENT_CLEAR_RE = /if \(dnd && dnd\.wcId === payload\.wcId\) \{\s*clearDragVisuals\(\);\s*dnd = null;\s*\}/;
/** Index of the silent-clear block at/after `fromIndex`, or -1. */
function silentClearIndex(source, fromIndex) {
  const m = source.slice(fromIndex).match(SILENT_CLEAR_RE);
...
test('AC5b: onTabMovedAway silently clears a live session whose tab is the departing one', () => {
  ...
  const silent = silentClearIndex(real, anchor);
  ...
  const removed = real.replace(new RegExp(`\\n[ \\t]*${SILENT_CLEAR_RE.source}`), '');
  assertMutated(real, removed, 'removed-silent-clear');
  assert.equal(
    silentClearIndex(removed, removed.indexOf('onTabMovedAway((payload)')),
    -1,
    'mutated → the silent-clear pin FAILS'
  );
});
